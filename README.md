...
- 多圈路线会基于已绘制路线重复生成，并加入**逐点平滑随机游走抖动**（±2m，自回归 α=0.92），使各圈轨迹自然偏移而不是整体平移。
- **标准操场预设**：一键在地图上生成标准 400m 椭圆跑道，并提供拖拽（✥）、旋转（↻）、缩放（⤢）三个交互把手，可精确贴合实际操场位置。
- **心率热身时长控制**：可设置心率爬升时间（分钟），模拟真实热身过程，平滑过渡至目标心率区间。
- **平滑轨迹插值**：后端使用 Catmull-Rom 样条曲线对路径点做 ~3m 间距密集插值，导出轨迹更流畅自然。
- **默认开始时间 20:30**：多份导出时每份在 ±10 分钟内随机抖动，各文件时间间隔一天。
...
- Node.js 18 或更高版本（已在 macOS 上使用 Node.js v25 测试通过）。
...
2. 在右侧地图上依次点击绘制跑步轨迹；或点击"**标准操场**"按钮自动生成 400m 椭圆跑道，再通过 ✥/↻/⤢ 把手拖拽到目标位置、调整角度和大小。
3. 在左侧填写静息心率、最大心率、**心率热身时长**、圈数和导出份数。
4. 在导出列表中为每一份文件设置开始时间和配速（默认 20:30，多份时自动加抖动）。
